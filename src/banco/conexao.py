...
class _ConexaoPostgres:
    ...
    def executemany(self, sql: str, seq_params: list) -> None:
        ...
        if not seq_params:
            return
        # Traduz uma vez (mesmo SQL pra todos) e remove o RETURNING id
        # (em lote não precisamos do id de volta — e RETURNING quebra executemany)
        sql_pg, _ = _traduzir_sql(sql, seq_params[0] if seq_params else ())
        if sql_pg is None:
            return
        sql_pg = re.sub(r"\s+RETURNING\s+id\s*$", "", sql_pg, flags=re.IGNORECASE)
        try:
...
def _conexao_viva(conn) -> bool:
    """
    Verifica se a conexão parece viva — SEM ir ao banco (evita round-trip extra).
    Para Postgres, basta checar o atributo 'closed' (local/instantâneo). Se a
    conexão tiver caído por timeout do Neon, o próprio execute()/executemany()
    reconecta automaticamente no primeiro erro. Para SQLite, a conexão local
    não expira.
    """
    try:
        if usar_postgres():
            ...
            if inner is None:
                return False
            return not getattr(inner, "closed", False)
        return True
    except Exception:
        return False
