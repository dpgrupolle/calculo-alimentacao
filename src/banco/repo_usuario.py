...
Sistema de aprovação por chave (igual ao esqueleto LLE):
  - Cadastro livre gera uma chave única (ex: K7P4-N2X9-B5M1).
  - A Gestão de RH libera o acesso aprovando pela tela "Usuários" OU
    colando a chave em [usuarios_aprovados].chaves nos Streamlit Secrets.
...
def gerar_chave_aprovacao() -> str:
    """Gera chave amigável XXXX-XXXX-XXXX (sem caracteres ambíguos)."""
...
def listar_chaves_aprovadas_do_secrets() -> set[str]:
    """
    Lê as chaves aprovadas dos Secrets:
        [usuarios_aprovados]
...
def sincronizar_aprovacoes_com_secrets() -> int:
    """Marca como aprovados os usuários cujas chaves estão no Secrets."""
...
def _row_para_usuario(row) -> Usuario:
    ...
    return Usuario(
        ...
        chave_aprovacao=row["chave_aprovacao"] if "chave_aprovacao" in chaves else None,
        aprovado=bool(row["aprovado"]) if "aprovado" in chaves else False,
        ultimo_login=row["ultimo_login"] if "ultimo_login" in chaves else None,
    )
...
def autenticar(email: str, senha: str) -> Optional[Usuario]:
    ...
    NÃO bloqueia por aprovado=0 aqui (a UI mostra mensagem amigável).
    """
    cur = obter_conexao().execute(
        "SELECT * FROM usuario WHERE LOWER(email) = LOWER(?);", (email,)
    )
...
def criar_usuario(nome: str, email: str, senha: str,
                  perfil: PerfilUsuario,
                  forcar_gestao_se_primeiro: bool = True) -> Usuario:
    """
    Cria novo usuário com chave de aprovação única.
    Primeiro usuário vira GESTAO_RH e é auto-aprovado (bootstrap).
    """
    nome = (nome or "").strip()
    ...
    if forcar_gestao_se_primeiro and eh_primeiro:
        perfil_efetivo = PerfilUsuario.GESTAO_RH

    # Gera chave única (tenta até 5 vezes)
    chave = None
    for _ in range(5):
    ...
    with transacao() as conn:
        cur = conn.execute(
            ...
            VALUES (?, ?, ?, ?, 1, 0, ?, ?);
            """,
            (nome, email, senha_hash, perfil_efetivo.value, chave,
             1 if auto_aprovado else 0),
        )
...
def redefinir_senha_temporaria(usuario_id: int, senha_temporaria: str) -> None:
    """Admin redefine senha; usuário é forçado a trocar no próximo login."""
    alterar_senha(usuario_id, senha_temporaria, deve_trocar_no_proximo_login=True)
...
def aprovar_usuario(usuario_id: int) -> None:
    with transacao() as conn:
        conn.execute("UPDATE usuario SET aprovado = 1 WHERE id = ?;", (usuario_id,))


def recusar_usuario(usuario_id: int) -> None:
    """Recusa: aprovado=0 e ativo=0 (impede login, preserva cadastro)."""
    with transacao() as conn:
        conn.execute(
            "UPDATE usuario SET aprovado = 0, ativo = 0 WHERE id = ?;", (usuario_id,)
        )
...
def alterar_nome(usuario_id: int, novo_nome: str) -> None:
    ...
    with transacao() as conn:
        conn.execute("UPDATE usuario SET nome = ? WHERE id = ?;", (novo_nome, usuario_id))


def alterar_perfil(usuario_id: int, novo_perfil: PerfilUsuario) -> None:
    """
    Altera o cargo. Não permite rebaixar o ÚLTIMO Gestão de RH ativo
    (senão o sistema fica sem ninguém pra gerenciar usuários).
    """
    ...
    if atual.perfil == PerfilUsuario.GESTAO_RH and novo_perfil != PerfilUsuario.GESTAO_RH:
        cur = obter_conexao().execute(
            "SELECT COUNT(*) FROM usuario "
            "WHERE perfil = 'GESTAO_RH' AND ativo = 1 AND aprovado = 1;"
        )
        if int(cur.fetchone()[0]) <= 1:
            raise ValueError(
                "Você não pode rebaixar este usuário — ele é a ÚLTIMA Gestão de RH "
                "ativa do sistema. Promova outro usuário antes de rebaixar este."
            )
